...
export default function ProductReviews({ productId }: ProductReviewsProps) {
  ...
  async function handleSubmitReview(e: React.FormEvent) {
    e.preventDefault();
    if (!user) {
      showToast('Jelentkezz be az értékeléshez!', 'info');
      return;
    }
    ...
    try {
      ...
      if (error) {
        if (error.code === '23505') {
          showToast('Már értékelted ezt a terméket!', 'info');
        } else {
          throw error;
        }
      } else {
        showToast('Köszönjük az értékelésed!', 'success');
  ...
  async function handleHelpful(reviewId: string) {
    ...
    try {
      ...
      if (error) {
        if (error.code === '23505') {
          showToast('Már hasznosnak jelölted ezt az értékelést!', 'info');
        } else {
          throw error;
        }
      } else {
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        {user && !showReviewForm && (
          <button
            ...
            className="px-6 py-2 bg-black text-white rounded-lg hover:bg-vintage-red/90 transition-colors"
          >
            Értékelés írása
          </button>
        )}
        ...
        <form onSubmit={handleSubmitReview} className="mb-8 p-6 bg-gray-50 rounded-lg">
          ...
          <div className="flex gap-2">
            <button
              type="submit"
              disabled={submitting}
              className="px-6 py-2 bg-black text-white rounded-lg hover:bg-vintage-red/90 transition-colors disabled:bg-gray-400"
            >
