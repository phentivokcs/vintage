...
export default function StoreReviewManagement() {
    ...
    try {
      ...
      const reviewsWithUsers = await Promise.all(
        (data || []).map(async (review) => {
          ...
          return {
            ...review,
            users: userData || { name: null, email: 'Ismeretlen' }
          };
        })
      );
  ...
  async function handleToggleStatus(review: StoreReview) {
    ...
    try {
      ...
      showToast('Hiba történt a státusz frissítése során!', 'error');
    }
  }
      ...
      {reviews.length === 0 ? (
        ...
        <div className="space-y-4">
          {reviews.map((review) => (
            <div
              key={review.id}
              className={`bg-vintage-cream-light border-vintage-beige/20 rounded-lg p-6 ${
