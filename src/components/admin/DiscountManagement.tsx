...
export default function DiscountManagement() {
  ...
  const updateDiscount = async (
    ...
    try {
      ...
      showToast('Hiba történt az akció mentésekor', 'error');
    } finally {
      setSaving(null);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {products.map((product) => {
          ...
          return (
            <div key={product.id} className="bg-vintage-cream-light border-vintage-beige/20 rounded-lg p-6">
...
function VariantDiscountForm({
  ...
  const handleSave = () => {
    ...
    if (price && price >= variant.price_gross) {
      showToast('Az akciós ár nem lehet nagyobb vagy egyenlő az eredeti árnál', 'error');
      return;
    }

    if (price && (!startDate || !endDate)) {
      showToast('Add meg az akció kezdő és befejező dátumát', 'error');
      return;
    }
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        ...
        <div className="flex items-end">
          <button
            onClick={handleSave}
            disabled={saving}
            className="w-full px-4 py-2 bg-vintage-red text-white rounded-lg hover:bg-vintage-red/90 disabled:bg-gray-400 flex items-center justify-center gap-2"
          >
            {saving ? (
              'Mentés...'
            ) : (
