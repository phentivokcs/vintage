...
import { Upload, Save, RotateCcw, ImageIcon, AlertCircle, Check } from 'lucide-react';
...
function HeroSection({ getValue, updateValue, assets, handleImageUpload, uploading }: any) {
  return (
    <div className="space-y-6">
...
function PromoSection({ getValue, updateValue, assets, handleImageUpload, uploading }: any) {
  return (
...
function CategorySection({ getValue, updateValue, assets, handleImageUpload, uploading }: any) {
  return (
    <div className="space-y-8">
      <h2 className="text-xl font-semibold">Kategória Header Képek</h2>

      {['men', 'women'].map((category) => (
        <div key={category} className="border border-gray-200 rounded-lg p-6 space-y-4">
          <h3 className="font-medium">
            {category === 'men' ? 'Férfi Ruházat' : 'Női Ruházat'}
          </h3>

          <ImageUpload
            assetKey={`category_${category}`}
            currentUrl={assets[`category_${category}`]?.public_url}
            onUpload={handleImageUpload}
            uploading={uploading[`category_${category}`]}
          />
          ...
          <div>
            ...
            <input
              type="text"
              value={getValue('category', `${category}_title`)}
              onChange={(e) => updateValue('category', `${category}_title`, e.target.value)}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              value={getValue('category', `${category}_description`)}
              onChange={(e) => updateValue('category', `${category}_description`, e.target.value)}
              rows={2}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg"
            />
          </div>
...
function SEOSection({ getValue, updateValue, assets, handleImageUpload, uploading }: any) {
  return (
    <div className="space-y-6">
...
function DesignSection({ getValue, updateValue, assets, handleImageUpload, uploading }: any) {
  return (
    <div className="space-y-6">
