...
import { Package, Eye, Truck, CheckCircle, FileText, Download, Box } from 'lucide-react';
...
export default function OrderManagement() {
  ...
  const updateOrderStatus = async (orderId: string, newStatus: string) => {
    ...
    if (error) {
      showToast('Hiba a státusz frissítésekor: ' + error.message, 'error');
    } else {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          showToast('Email értesítés elküldve!', 'success');
        }
      } catch (emailError) {
  ...
  const generateInvoice = async (orderId: string) => {
    ...
    try {
      const apiUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/billingo-invoice`;
      const response = await fetch(apiUrl, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${import.meta.env.VITE_SUPABASE_ANON_KEY}`,
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Számlakiállítás sikertelen');
      }

      const result = await response.json();

      showToast('Számla sikeresen kiállítva!', 'success');
      fetchOrders();

      return result;
    } catch (error) {
      console.error('Számlakiállítási hiba:', error);
      showToast(
        error instanceof Error ? error.message : 'Hiba a számlakiállítás során',
        'error'
      );
    } finally {
  ...
  const generateShippingLabel = async (orderId: string, provider: 'dpd' | 'packeta') => {
    setShippingLoading(orderId);

    try {
      const functionName = provider === 'dpd' ? 'dpd-shipping' : 'packeta-shipping';
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Címke generálás sikertelen');
      }

      const result = await response.json();

      showToast(`${provider.toUpperCase()} címke sikeresen generálva!`, 'success');
      fetchOrders();

      return result;
    } catch (error) {
      console.error('Címke generálási hiba:', error);
      showToast(
        error instanceof Error ? error.message : 'Hiba a címke generálás során',
        'error'
      );
    } finally {
  ...
  const exportToCSV = () => {
    const csvData = orders.map(order => ({
      'Rendelés ID': order.id.slice(0, 8),
      'Email': order.users?.email || 'N/A',
      'Státusz': getStatusLabel(order.status),
      'Fizetési státusz': order.payment_status,
      'Összeg': order.total_gross,
      'Dátum': formatDate(order.created_at),
      'Számlaszám': order.invoice_number || '-',
      'Szállítási cím': `${order.shipping_address?.name}, ${order.shipping_address?.address}, ${order.shipping_address?.city}`,
    }));
    ...
    showToast('CSV exportálva!', 'success');
  };
  ...
  const bulkUpdateStatus = async (newStatus: string) => {
    ...
    if (selectedOrderIds.length === 0) {
      showToast('Nincs kiválasztott rendelés', 'error');
      return;
    }
    ...
    try {
      ...
      showToast(`${selectedOrderIds.length} rendelés státusza frissítve!`, 'success');
      setSelectedOrders(new Set());
      fetchOrders();
    } catch (error) {
      console.error('Bulk update error:', error);
      showToast('Hiba a tömeges frissítés során', 'error');
    }
  };
    ...
    <div className="space-y-6">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-blue-900">
              {selectedOrders.size} rendelés kiválasztva
            </p>
            <div className="flex gap-2">
              <button
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-6">
            {orders.map((order) => (
              <div key={order.id} className="bg-vintage-cream-light rounded-lg border border-gray-200 p-6">
                ...
                {order.status === 'pending' && (
                  <button
                    ...
                    className="px-4 py-2 bg-vintage-red text-white rounded-lg hover:bg-vintage-red/90 text-sm"
                  >
                    Fizetve jelölés
                  </button>
                )}
                    ...
                    Törlés
                      ...
                      {order.payment_status === 'paid' && !order.invoice_number && (
                        ...
                        >
                          ...
                          {invoiceLoading === order.id ? 'Kiállítás...' : 'Számla kiállítása'}
                        </button>
                      )}
                      ...
                      {order.invoice_number && (
                        <div className="flex items-center gap-2 px-4 py-2 bg-green-50 border border-green-200 rounded-lg text-sm">
                          ...
                          <span className="text-green-700">
                            Számla: {order.invoice_number}
                          </span>
                        </div>
                      )}

                      {order.status === 'processing' && !order.shipping_label_url && (
                        <>
                          <button
                            onClick={() => generateShippingLabel(order.id, 'dpd')}
                            disabled={shippingLoading === order.id}
                            className="px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 text-sm flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            <Box className="w-4 h-4" />
                            {shippingLoading === order.id ? 'Generálás...' : 'DPD Címke'}
                          </button>
                          <button
                            onClick={() => generateShippingLabel(order.id, 'packeta')}
                            disabled={shippingLoading === order.id}
                            className="px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 text-sm flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            <Box className="w-4 h-4" />
                            {shippingLoading === order.id ? 'Generálás...' : 'Packeta Címke'}
                          </button>
                        </>
                      )}
                      ...
                      {order.shipping_label_url && (
                        <div className="flex items-center gap-2 px-4 py-2 bg-orange-50 border border-orange-200 rounded-lg text-sm">
                          ...
                          <span className="text-orange-700">
                            Címke: {order.shipping_provider?.toUpperCase()}
                          </span>
                          <a
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-vintage-cream-light rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
