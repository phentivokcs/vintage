    ...
    <a href={`/p/${product.slug}`} className="group block">
      <div className="relative overflow-hidden rounded-lg bg-gray-100 aspect-[3/4]">
        ...
        <button
          ...
          className="absolute top-3 right-3 p-2 bg-vintage-cream-light/90 backdrop-blur-sm rounded-full shadow-md hover:bg-vintage-cream transition-colors z-10 flex items-center gap-1.5 border border-vintage-beige/20"
        >
          <Heart
            className={`w-5 h-5 transition-colors ${inWishlist ? 'fill-vintage-red text-vintage-red' : 'text-vintage-beige'}`}
          />
          {wishlistCount > 0 && (
            <span className="text-xs font-semibold text-vintage-beige">{wishlistCount}</span>
          )}
        </button>

        {hasActiveDiscount && originalPrice && displayPrice && (
          <div className="absolute top-3 left-3 px-3 py-1 bg-vintage-red text-white text-xs font-bold rounded-full shadow-md">
            -{calculateDiscountPercent(originalPrice, displayPrice)}%
          </div>
        )}

        {mainVariant?.is_unique_piece && !hasActiveDiscount && (
          <div className="absolute top-3 left-3 px-3 py-1 bg-vintage-beige text-white text-xs font-semibold rounded-full shadow-md">
            Egyedi darab
          </div>
        )}

        {product.condition && (
          <div className="absolute bottom-3 left-3 px-3 py-1 bg-vintage-cream-light/90 backdrop-blur-sm text-vintage-beige text-xs font-medium rounded-full border border-vintage-beige/30 shadow-sm">
