      ...
      contentData.data?.forEach((item: any) => {
        ...
        switch (item.section) {
          case 'hero':
            ...
            if (item.key === 'banner_image') newContent.hero.bannerImage = value;
            break;
          case 'promo':
          ...
          case 'category':
            ...
            if (item.key === 'tops_image') newContent.category.tops.image = value;
            ...
            if (item.key === 'accessories_image') newContent.category.accessories.image = value;
            ...
            if (item.key === 'shoes_image') newContent.category.shoes.image = value;
            break;
          case 'footer':
          ...
          case 'design':
            ...
            if (item.key === 'logo') newContent.design.logo = value;
            break;
        }
      });

      const heroBanner = assetsMap.get('hero_banner');
      if (heroBanner) newContent.hero.bannerImage = heroBanner;

      const promo1 = assetsMap.get('promo_1');
      if (promo1) newContent.promo.banner1.image = promo1;

      const promo2 = assetsMap.get('promo_2');
      if (promo2) newContent.promo.banner2.image = promo2;

      const promo3 = assetsMap.get('promo_3');
      if (promo3) newContent.promo.banner3.image = promo3;

      const categoryMen = assetsMap.get('category_men');
      if (categoryMen) newContent.category.men.headerImage = categoryMen;

      const categoryWomen = assetsMap.get('category_women');
      if (categoryWomen) newContent.category.women.headerImage = categoryWomen;

      const categoryTops = assetsMap.get('category_tops');
      if (categoryTops) newContent.category.tops.image = categoryTops;

      const categoryAccessories = assetsMap.get('category_accessories');
      if (categoryAccessories) newContent.category.accessories.image = categoryAccessories;

      const categoryShoes = assetsMap.get('category_shoes');
      if (categoryShoes) newContent.category.shoes.image = categoryShoes;

      const ogImage = assetsMap.get('og_image');
      if (ogImage) newContent.seo.ogImage = ogImage;

      const logoFromAssets = assetsMap.get('logo');
      if (logoFromAssets) {
        newContent.design.logo = logoFromAssets;
      }
